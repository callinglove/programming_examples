...
function! s:cmake_configure(cmake_vim_command_args)
  ...
  let s:cmd = g:cmake_command.' '.l:home_dir.' '.l:build_dir_path.' '.l:argumentstr . " " . join(a:cmake_vim_command_args)
...
function! ListTargets(A, L, C)
    ...
    let all_targets = split(system(g:cmake_command . " --build ". b:build_dir . " --target help | awk ' NR > 1 {print $2}'"), '\n')
...
function! s:cmake(...)
  ...
  let &makeprg = g:cmake_command . ' --build ' . shellescape(b:build_dir) . l:smp . ' --target'
