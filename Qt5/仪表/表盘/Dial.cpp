...
#define  BigBottom       3
    ...
    {
        qreal currentValue =(qreal)scale*((maxvalue-minvalue)/24);

        if((currentValue>(value-minvalue)&&(paint.brush().color()!=ScaleColor)))
        {
            paint.setBrush(ScaleColor);
        }
        else if((currentValue<=(value-minvalue)&&(paint.brush().color()!=slideScaleColor)))
        {
...
void Dial::drawScaleTextColor(QPainter &paint)    //绘制刻度值
{
    ...
    for(int i=0;i<7;i++)              //设置7个点
    {
        ...
        qreal curValue =(qreal)i*((maxvalue-minvalue)/6)+minvalue;
        if((curValue>(value)&&(paint.pen().color()!=ScaleColor)))
        {
            paint.setPen(ScaleColor);
        }
        else if((curValue<=(value)&&(paint.pen().color()!=slideScaleColor)))
        {
            paint.setPen(slideScaleColor);
        }

        QString values= fetchDecimalPoint(text.arg(curValue),Scale_decimal);

        qreal xoffset = (qreal)values.length()*size/2.0;
...
{
    // pie的角度范围是210~-30共240度
    int startAngle= 210*16-(int)(((value-minvalue)/(maxvalue-minvalue))*240*16);
    int spanAngle  =  210*16 - startAngle;
    ...
    paint.drawPie(QRectF((qreal)-radius*0.90,(qreal)-radius*0.90,(qreal)radius*1.80,(qreal)radius*1.80),startAngle,spanAngle);
...
{
    qreal angles[GradientSlideColor.count()];
    ...
    paint.setPen(Qt::NoPen);
    paint.setBrush(Conical);
    paint.drawPie(QRectF((qreal)-radius*top,(qreal)-radius*top,(qreal)radius*top*2,(qreal)radius*top*2),startAngle,spanAngle);

    paint.setBrush(bkColor);
    paint.drawPie(QRectF((qreal)-radius*bottom,(qreal)-radius*bottom,(qreal)radius*bottom*2,(qreal)radius*bottom*2),startAngle,spanAngle);
...
void Dial::drawDialShade(QPainter &paint,qreal Angle_start,qreal Angle_end,qreal ratio,int alpha)//给整个表盘绘制阴影
{
     ...
     QPointF  point[size*2];


     qreal  Round_Radius = radius*0.90;
     ...
     qreal top_Angle_spacing =  (Angle_End-Angle_Start)/size;
     ...
     else              //扁的圆环
     {
       ...
       {
          qreal currentBottomX= point[size-1].x()+xCurrentStep;

          qreal currentBottomY=point[size-1].y()+yCurrentStep;

          qreal topX = point[size-i+size-1].x();
...
void Dial::ontimeout()
{
    if(value != endvalue)
    {
        if( qAbs(value-endvalue)<=qAbs(timer_value))
        {
            value = endvalue;
            send();
        }
...
bool Dial::setEndValue(qreal v)           //设置终点值
{
    ...
    if(timer_msec==0 || timer_value == 0)           //表示定时器不需要延时,直接跳到终点
    {
        value = v;
