...
/*  自定义表盘
 *  作者: LifeYx
 *  博客: https://www.cnblogs.com/lifexy/p/9245918.html
 *  介绍:
 *      内部自动设定组件的数字,指针等大小,可以随意变大变小
 *      内部集成定时器,可以通过setTimerType()实现动画旋转快慢
 *      通过 setGradientColorMode() 可以自定义渐变颜色,通过 setSingleColorMode() 自定义单一颜色
 *      支持负数到正数更改
 *      支持小数点位数设置
...
public:
    ...
    /*设置圆组件颜色*/
    void setRoundColor(QColor obkColor,QColor bkColor,QColor centercolor);
    ...
    void setTextColor(QColor ScaleColor,QColor slideScaleColor,QColor ValueColor,QColor LabelColor);
