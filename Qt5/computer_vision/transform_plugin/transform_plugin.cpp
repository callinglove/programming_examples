        ...
        for (int i = 0; i < inputImage.rows; i++){
            for (int j = 0; j < inputImage.cols; j++){
                ...
                case 0://图像映射，缩小一半，居中显示
                    if(i>=inputImage.rows*0.25 && i<inputImage.rows*0.75 && j>=inputImage.cols*0.25 && j<inputImage.cols*0.75){
                        mapX.at<float>(i,j) = 2 * (j - inputImage.cols*0.25);//插值，宽高方向上跳一个像素取一个值
                        mapY.at<float>(i,j) = 2 * (i - inputImage.rows*0.25);
                    }else{
                        mapX.at<float>(i, j) = 0;//这里赋值两个0，表示原图(0,0)坐标位置的像素值，而不是表示黑色
                        mapY.at<float>(i, j) = 0;
                    }
                    break;
                case 1://左右镜像
                    mapX.at<float>(i,j)=inputImage.cols - j - 1;
                    mapY.at<float>(i,j)=i;
                    break;
                case 2://上下镜像
                    mapX.at<float>(i,j)=j;
                    mapY.at<float>(i,j)=inputImage.rows - i - 1;
                    break;
                case 3://左右上下都镜像
                    mapX.at<float>(i,j)=inputImage.cols - j - 1;
                    mapY.at<float>(i,j)=inputImage.rows - i - 1;
