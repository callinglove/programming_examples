...
Cv310Text::Cv310Text(const char *freeType, int font_scale)
{
	assert(freeType != NULL);
	...
	CvScalar fontSize;
	// 字体大小/空白比例/间隔比例/旋转角度
	fontSize.val[0] = font_scale;
	fontSize.val[1] = 0.5;
	fontSize.val[2] = 0.1;
	fontSize.val[3] = 0;
	float p = 1.0;

	setFont(NULL, &fontSize, NULL, &p);
}
...
void Cv310Text::setPath(const char *freeType)
{
	assert(freeType != NULL);
...
cv::Size Cv310Text::getTextSize(const char *text)
{
    cv::Size size;
    if (text == NULL) 
        return size;
...
void Cv310Text::updateSize(wchar_t wc, cv::Size& size)
{
    ...
    size.width += (int)((cols ? cols : space) + sep);
    size.height = size.height > rows ? size.height : rows;
}
...
{
	// 根据unicode生成字体的二值位图
	IplImage temp = (IplImage)frame; // modified by ZT 20170922
	IplImage* img = &temp;
//	IplImage* img = NULL;
//	img = &(IplImage)frame;
